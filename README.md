...
1. 下载上表对应文件，在客户端中作为本地配置导入。
2. 按下表填入自己的订阅。文中的 `https://YOUR_SUBSCRIPTION_URL` 只是占位，不是有效地址。
3. 启用配置后，打开节点订阅与远程规则的自动更新。
...
不同客户端支持的订阅格式不完全相同，同一条链接不保证五端通用。自建节点请写在各端的 `[Proxy]` / `proxies` 中，不要提交到公开仓库。
